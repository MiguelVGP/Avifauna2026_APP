...
if section == "🐦‍⬛ Visão geral":
    ...
    import numpy as np
    import plotly.graph_objects as go
    from pathlib import Path  # ✅ NECESSÁRIO (corrige NameError)
    ...
    if df_amostras.empty or any(c not in df_amostras.columns for c in [LOCAL_COL, SPEC_COL, INDIV_COL]):
        ...
        if base.empty:
            ...
            # =========================
            MAX_DIAM_PX = 380    # antes 280 (bolha maior cresce)
            PX_TO_X     = 0.025  # mantém
            MIN_GAP_PX  = 28     # antes 57 (bolhas bem mais juntas)
            SIZEMIN_PX  = 30     # antes 22 (as pequenas deixam de ser microscópicas)
            OUTLINE_W   = 2.5
            ...
            # Passer domesticus
            try:
                passer_path = ASSETS_IMG / "passer_domesticus.jpg"
                species_images["passer domesticus"] = image_to_circular_data_uri(str(passer_path), out_px=420)
            except Exception:
                pass

            # Columba livia  ✅ (novo)
            try:
                columba_path = ASSETS_IMG / "columba_livia.jpg"   # <-- coloca aqui o ficheiro
                species_images["columba livia"] = image_to_circular_data_uri(str(columba_path), out_px=420)
            except Exception:
                pass

            try:
                turdus_path = ASSETS_IMG / "turdus_merula.jpg"   # <-- coloca aqui o ficheiro
                species_images["turdus merula"] = image_to_circular_data_uri(str(turdus_path), out_px=420)
            except Exception:
                pass

            try:
                larussp_path = ASSETS_IMG / "larus_sp.jpg"   # <-- coloca aqui o ficheiro
                species_images["larus sp"] = image_to_circular_data_uri(str(larussp_path), out_px=420)
            except Exception:
                pass
            ...
            try:
                larus_fuscus_path = ASSETS_IMG / "larus_fuscus.jpg"   # <-- coloca aqui o ficheiro
            ...
            for _, r in agg.iterrows():
                ...
                # =========================
                # Texto adaptativo ao tamanho da bolha (SEM duplicações)
                # =========================
                ...
                for _, r in agg.iterrows():
                    ...
                    r_u = float(r["r_units"])
                    d_u = 2.0 * r_u  # diâmetro em unidades do eixo (já escalado)
                
                    # Fonte base (mais visível)
                    font_size = clamp(int(d_u * 3.0), 12, 26)
                
                    # Conteúdo por tamanho: garantir que em bolhas muito pequenas aparece pelo menos o nome
                    if d_u < 1.55:
